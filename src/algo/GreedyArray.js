...
var GreedyArray = function(){
  ...
  var objects = ['Apple','Dell'], criteria = ['design', 'performance'];
  ...
  for(var kr = 0; kr < knownRes.length; kr++) console.log(JSON.stringify(knownRes[kr]) + "\n");
     console.log('---------------------------------');
  ...
  baseN.forEach(function(a){
    ...
    for(var al = 0; al < a.length; al++) {
      var w = {
        ...
        'sign' : a[al]
          ...
          var tmpSign = "<";
          if(w.sign == "gt") tmpSign = ">"; else if(w.sign == "lt") tmpSign = "<"; else tmpSign = "~";
            console.log(""+w.object1 +", " + w.object2 + ", " + w.criterion+", " + tmpSign);
      ...
      if(dominanceCountDict[realKey].count == criteria.length && dominanceCountDict[realKey].isvalid == 1) {
        ...
        if(("gt".indexOf(dominanceCountDict[realKey].sign) > -1)) {
    ...
    console.log("\n    world "+count+": " + (math.round((prob * divi), 3)) + "  ");
    ...
    if(ENABLE_LOGGING) for(var y = 0; y < world.length; y++) console.log("world " + y + " -> " + JSON.stringify(world[y]));
    ...
    for(var m = 0; m < pObjs.length; m++) {
      ...
      if(true || ENABLE_LOGGING) console.log("" + a + " -> " + pObjs[m]);
    }

    if(pObjs.length == 0) {
      if(true || ENABLE_LOGGING) console.log("no pareto optimal objects");
  ...
  if(ENABLE_LOGGING) console.log("--------FINAL dict -> " + JSON.stringify(dict));
