...
path = require('path'),
nconf = require('nconf'),
makeSource = require('stream-json'),
Assembler  = require("stream-json/utils/Assembler"),
https = require('https'),
util = require('util'),
...
var PRUNING_THRESHOLD = nconf.get("PRUNING_THRESHOLD");
    ...
    for(var g = 0; g < combination.length; g++) {
      for(var h = 0; h < resp.length; h++) {
        if((combination[g].object1 == resp[h].object1 &&
          ...
          (combination[g].object1 == resp[h].object2 &&
            combination[g].object2 == resp[h].object1 &&
            combination[g].criterion == resp[h].criterion)) {
              pos.push(g);
            }
          }
        ...
        var counter = {'undefined' : 0};
        for(var obj in objects) counter[obj] = 0;
        ...
        var z, zeroProbQuesCount = 0;;

        for(var eac in resp) {
           delete resp[eac].candidates;
           if(resp[eac].gt <= PRUNING_THRESHOLD) zeroProbQuesCount++;
           if(resp[eac].lt <= PRUNING_THRESHOLD) zeroProbQuesCount++;
           if(resp[eac].indiff <= PRUNING_THRESHOLD) zeroProbQuesCount++;
        }
        console.log("zero prob question: " + zeroProbQuesCount);
        // console.log(JSON.stringify(resp));

        // loop through the possible worlds but only within limits
        for(z = startIndex; z < abc; z++){
          // stop the loop when the last element is reached
          if(z >= subWorldLength)  break;
          ...
          for(var al = 0; al < a.length; al++) {
            ...
            if(pos.indexOf(al) > -1) {
              ...
              for(var n = 0; n < resp.length; n++) {
                if(w.object1 == resp[n].object1 &&
                  ...
                  w.criterion == resp[n].criterion) {
                    ...
                    if(resp[n][w.sign] <= PRUNING_THRESHOLD) isWorldProbZero = true;
                    break;
                  }
                }
              }
              ...
              if(isWorldProbZero && counterZeroBool) {
                zeroWorldCount++;
                counterZeroBool = false;
                continue;
              }
          ...
          var ranksWriter = function(){
            ...
            ws1.on('finish', function(){console.log("Writing ranks is Finished");})
            ws1.end();
          };
          ...
          if(startIndex > 0) {
            ...
            // using optimzed way of reading big object
...
// END 1
            ...
            if(fs.existsSync(oFileName))
            fs.createReadStream(path.resolve(oFileName)).pipe(source.input);
            else console.log("Oops! The file '"+oFileName+"' doesn't seem to exist");
            /*
      // START 2
          ...
          console.log("Invalid JSON file. The file might be too large to read too :(");
        }
      } else {
